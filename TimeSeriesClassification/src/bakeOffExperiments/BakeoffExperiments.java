...
import new_COTE_experiments.FixedIntervalForest;
 ...
 * @author ajb
 */
public class BakeoffExperiments extends Thread{
   Instances train;
   Instances test;
//For threaded version only
   ...
   String classifier;
    ...
    public BakeoffExperiments(Instances tr, Instances te, String cls, String prob, String predictions,int res, int f){
        train=tr;
        test=te;
        classifier=cls;
        problem=prob;
...
//All classifierName names  
    ...
    static String[] standard={"NB","C45","SVML","SVMQ","BN","RandF","RotF","MLP"};
...
//Limit the transform to three days                
                ((ST_Ensemble)c).setTimeLimit(3*ShapeletTransformFactory.dayNano);
                break;
            case "DTW":
                c=new DTW_1NN();
                ((DTW_1NN)c).setR(1.0);
...
//                throw new Exception("Unknown classifierName "+classifierName);
 ...
 //Do all the reps for one problem   
    public static void localThreadedRun(String[] args) throws Exception{
        ...
        BakeoffExperiments[] thr=new BakeoffExperiments[reps-start];
        for (int i = start; i < reps; i++) {
            Instances[] data=InstanceTools.resampleTrainAndTestInstances(train,test, i);
            thr[i-start]=new BakeoffExperiments(data[0],data[1],classifier,problem,predictions,1,i);
        }
...
//                Instances[] data=InstanceTools.resampleTrainAndTestInstances(train,test, i);
            ...
            if(f.exists()){
                ...
                if(size<CollateResults.testSizes[i]){
                    ...
                    Instances train=ClassifierTools.loadData(DataSets.problemPath+problem+"/"+problem+"_TRAIN");
                    Instances test=ClassifierTools.loadData(DataSets.problemPath+problem+"/"+problem+"_TEST");
                    exp[i]=new BakeoffExperiments(train,test,classifier,problem,predictions,1,0);
...
//        String problem=unfinished[Integer.parseInt(args[1])-1];
        ...
        if(s.equals("ACF")){
            ...
            ((PSACF_Ensemble) classifier).doTransform(false);
        }else if(s.equals("PS")){
