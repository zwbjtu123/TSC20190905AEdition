...
//                throw new Exception("Unknown classifier "+classifier);
    ...
    public static void clusterSingleClassifierAndFold(String[] args){
//first gives the problem file      
        String classifier=args[0];
        String s=args[1];
...
//Check whether fold already exists, if so, dont do it, just quit
        ...
        if(!f.exists()){
      //      of.writeString(s+","); );
            double acc =ClusterClassifierExperiment.singleSampleExperiment(train,test,c,fold,predictions);
 //       of.writeString("\n");
        }
    }
    ...
    public static void main(String[] args) throws Exception{
        if(args.length==2)
            ...
            clusterSingleClassifierAndFold(args);
        System.exit(0);
        Classifier c;
