...
public class DD_DTW_EfficientWithoutKNN implements Classifier{
...
//    protected boolean paramsSet;
    ...
    private DTW_DistanceBasic dtw;
    
    public DD_DTW_EfficientWithoutKNN(){
        paramsSet = false;
        dtw = new DTW_DistanceBasic();
    }
    
    public DD_DTW_EfficientWithoutKNN(double a, double b){
        this.a = a;
        this.b = b;
        dtw = new DTW_DistanceBasic();
    }
    ...
    public void buildClassifier(Instances train) throws Exception{
        ...
        this.train = train;
        this.derTrain = new GoreckiDerivativeFilter().process(train);
        
        if(!paramsSet){
            this.crossValidateForAandB();
            paramsSet=true;
        }
    }
    

    public double[] crossValidateForAandB(){
...
//            double[] individualDistances;
        ...
        for(int i = 0; i < n; i++){
            ...
            for(int j = 0; j < n; j++){
                ...
                dist = Math.sqrt(dtw.distance(this.train.instance(i), this.train.instance(j)));
                dDist = Math.sqrt(dtw.distance(this.derTrain.instance(i), this.derTrain.instance(j)));
    ...
    public double classifyInstance(Instance instance) throws Exception{
        ...
        Instances temp = new Instances(this.train,1);
        temp.add(instance);
        temp = new GoreckiDerivativeFilter().process(temp);
        
        double raw, der, thisDist;
        
        for(int i = 0; i < this.train.numInstances(); i++){
            raw = Math.sqrt(dtw.distance(train.instance(i), instance));
            der = Math.sqrt(dtw.distance(derTrain.instance(i), temp.instance(0)));
            thisDist = this.a*raw+this.b*der;
            
            if(thisDist < bsfDistance){
                bsfDistance = thisDist;
                bsfClass = train.instance(i).classValue();
            }
        }
...
//            System.out.print(df.format(err)+",");
            ...
            // DDDTW
//            DD_DTW_Efficient_withoutKNN dd_dtw = new DD_DTW_Efficient_withoutKNN();
//            correct = getCorrect(dd_dtw, train, test);
//            acc = (double)correct/test.numInstances();
//            err = (1-acc)*100;
//            System.out.println(df.format(err));
            DD_DTW_EfficientWithoutKNN dd_dtw = new DD_DTW_EfficientWithoutKNN();
            correct = getCorrect(dd_dtw, train, test);
            acc = (double)correct/test.numInstances();
            err = (1-acc)*100;
            System.out.println(df.format(err));
    ...
    public static void main(String[] args){
        ...
        try{
            if(option==1){
                ...
                DD_DTW_EfficientWithoutKNN nndw = new DD_DTW_EfficientWithoutKNN();;
...
//                String dataName = "ItalyPowerDemand";
                ...
                long total = 0;
                for(int i = 0; i < 11; i++){
                    DD_DTW_EfficientWithoutKNN nntw = new DD_DTW_EfficientWithoutKNN();
...
//                String dataName = "ItalyPowerDemand";
                ...
                for(int i = 0; i < 11; i++){
                    ...
                    for(int j = 0; j < test.numInstances();j++){
                        nntw.classifyInstance(test.instance(j));
                    }
                    long endTime = System.nanoTime();
                    if(i>0){ // unfair Java overheads added into the first run, so knock that one out before considering timing
                        total+=endTime-startTime;
    ...
    protected static int getCorrect(DD_DTW_EfficientWithoutKNN classifier, Instances train, Instances test) throws Exception{
