...
/**
 * An approximate filter to transform a dataset by k shapelets. The
 * approximation is achieved by means of sampling the dataset according to
    ...
    public Instances process(Instances dataInst) throws IllegalArgumentException {
        ...
        for (int i = 0; i < shapelets.size() + 1; i++) {
            ...
            if (i < shapelets.size()) {
                ...
                //if the stats is not created use the normal method.
            ...
            for (int j = 0; j < dataInst.numInstances(); j++) {
                if (i < shapelets.size()) {
                    ...
                    if (data != null && stats != null) {
                        ...
                        dist = subseqDistance.calculate(dataInst.get(j).toDoubleArray());
                    }
