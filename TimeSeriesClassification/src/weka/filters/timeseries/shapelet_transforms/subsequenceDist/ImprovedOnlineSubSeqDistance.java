...
public class ImprovedOnlineSubSeqDistance extends OnlineSubSeqDistance{
    ...
    public double calculate2(double[] timeSeries, int timeSeriesId)
    {
        DoubleWrapper sumPointer = new DoubleWrapper();
        ...
        double[] subseq = new double[candidate.length];
        System.arraycopy(timeSeries, timeSeries.length - candidate.length, subseq, 0, subseq.length);
        ...
        for (int i = timeSeries.length - candidate.length - 1; i > 0; i--) {
            //Update the running sums
            start = timeSeries[i];
            end = timeSeries[i + candidate.length];
            
            //get rid of the end and add on the start.
            sum += start - end;
            sumsq += (start * start) - (end * end);
