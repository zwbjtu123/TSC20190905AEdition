...
//		Instances fftTest,fftTrain;
		...
		for(int i=0;i<DataSets.fileNames.length;i++){
			System.out.println(" Running data set "+DataSets.fileNames[i]);
                        
	//1. Load test/train split
			String base=path+DataSets.fileNames[i]+"\\"
...
//				dtwAcc=ClassifierTools.singleTrainTestSplitAccuracy(c,train,test);
			...
			//4. Write to file
			of.writeLine(DataSets.fileNames[i]+","+euclidAcc+","+acfAcc+","+pacfAcc+","+","+armaAcc+","+histAcc[0]+","+histAcc[1]+","+histAcc[2]+","+histAcc[3]);
		}
...
//Fit Histogram		
	...
	public static void runLengthTest(){
		for(int i=0;i<DataSets.fileNames.length;i++)
		{
			Instances test=ClassifierTools.loadData(DataSets.dropboxPath+DataSets.fileNames[i]+"\\"+DataSets.fileNames[i]+"_TEST.arff");
			Instances train=ClassifierTools.loadData(DataSets.dropboxPath+DataSets.fileNames[i]+"\\"+DataSets.fileNames[i]+"_TRAIN.arff");
...
/*			all[0] = new IBk(1);
		all[1]=new kNN(new DTW_DistanceEfficient());
	all[3]=new DTW_1NN();
		((DTW_1NN)all[3]).optimiseWindow(true);
*/
